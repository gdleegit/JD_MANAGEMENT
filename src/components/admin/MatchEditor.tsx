...
const RECORD_TYPES = [
  { value: "GOAL",     label: "일반골", emoji: "⚽" },
  { value: "OWN_GOAL", label: "자책골", emoji: "⚽" },
...
export default function MatchEditor({ match, tournament, onBack }: { match: Match; tournament: Tournament; onBack: () => void }) {
  ...
  const [recordForm, setRecordForm] = useState({ teamId: match.homeTeam.id, playerId: "", minute: "", half: "1", type: "GOAL" });
  ...
  // 골 → 로컬 스테이징, 카드 → API 즉시 호출
  const addRecord = async () => {
    if (isCardType(recordForm.type)) {
      const res = await fetch(`/api/matches/${match.id}/cards`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          teamId: recordForm.teamId,
          playerId: recordForm.playerId || null,
          minute: recordForm.minute ? parseInt(recordForm.minute) : null,
          half: parseInt(recordForm.half),
          type: recordForm.type,
        }),
      });
      const { card } = await res.json();
      setCurrentMatch(m => ({ ...m, cards: [...m.cards, card].sort((a, b) => (a.minute ?? 999) - (b.minute ?? 999)) }));
      setRecordForm(f => ({ ...f, playerId: "", minute: "" }));
    } else {
      setPendingGoals(prev => [...prev, { ...recordForm, tempId: `${Date.now()}-${Math.random()}` }]);
      setRecordForm(f => ({ ...f, playerId: "", minute: "" }));
    }
  };
  ...
  const selectedTeam = recordForm.teamId === match.homeTeam.id ? match.homeTeam : match.awayTeam;
  ...
  const hasAnyRecord = totalGoals > 0 || currentMatch.cards.length > 0;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── 기록 추가 ── */}
      <div className="card p-5">
        <h4 className="font-bold mb-4">기록 추가</h4>
        ...
        <div className="flex gap-2 mb-3">
          {[match.homeTeam, match.awayTeam].map((t) => (
            <button
              key={t.id}
              onClick={() => setRecordForm({ ...recordForm, teamId: t.id, playerId: "" })}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium border transition-all flex-1 justify-center ${
                recordForm.teamId === t.id
                  ? "text-white border-transparent shadow-sm"
                  : "bg-white text-gray-500 border-gray-200 hover:border-gray-300"
              }`}
              style={recordForm.teamId === t.id ? { backgroundColor: t.color || "#3b82f6", borderColor: t.color || "#3b82f6" } : {}}
            >
              <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: recordForm.teamId === t.id ? "white" : (t.color || "#3b82f6"), opacity: recordForm.teamId === t.id ? 0.7 : 1 }} />
              {t.name}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="col-span-2 sm:col-span-1">
            ...
            <select className="input" value={recordForm.playerId} onChange={(e) => setRecordForm({ ...recordForm, playerId: e.target.value })}>
          ...
          <div>
            ...
            <div className="flex gap-1.5">
              {[{ value: "1", label: "전반" }, { value: "2", label: "후반" }].map((h) => (
                <button
                  key={h.value}
                  onClick={() => setRecordForm({ ...recordForm, half: h.value })}
                  className={`flex-1 py-2 rounded-lg text-xs font-medium border transition-all ${
                    recordForm.half === h.value
          ...
          <div>
            ...
            <input
              ...
              className="input"
              placeholder="예: 45"
              value={recordForm.minute}
              onChange={(e) => setRecordForm({ ...recordForm, minute: e.target.value })}
            />
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-1">
              {RECORD_TYPES.map((t) => (
                <button
                  key={t.value}
                  onClick={() => setRecordForm({ ...recordForm, type: t.value })}
                  className={`py-2 rounded-lg text-xs font-medium border transition-all ${
                    recordForm.type === t.value
                      ? "bg-blue-600 text-white border-blue-600"
                      : "bg-white text-gray-500 border-gray-200 hover:border-gray-300"
                  }`}
                >
        ...
        <button onClick={addRecord} className="btn btn-secondary w-full mt-3">
          {isCardType(recordForm.type) ? "+ 카드 기록" : "+ 목록에 추가"}
        </button>
      </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <h4 className="font-bold">
              경기 기록 ({currentMatch.goals.length}골
              {pendingGoals.length > 0 && <span className="text-amber-600"> + {pendingGoals.length} 미저장</span>}
              {currentMatch.cards.length > 0 && <span className="text-yellow-600"> · 카드 {currentMatch.cards.length}</span>})
            </h4>
          </div>
          ...
          {currentMatch.cards.length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-100">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">경고·퇴장</p>
              <div className="grid grid-cols-2 gap-4">
